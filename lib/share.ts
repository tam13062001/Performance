...
export async function createShareLink(params: {
    ...
    try {
        ...
        const insertRes = await client.query(
            `INSERT INTO ad_share_links (project_id, slug, label, allowed_pages, theme_primary, theme_secondary, theme_accent)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, slug, label, allowed_pages, theme_primary, theme_secondary, theme_accent, created_at, revoked_at`,
            [
                project.id,
                slug,
                params.label ?? null,
                params.allowedPages,
                params.theme?.primary ?? null,
                params.theme?.secondary ?? null,
                params.theme?.accent ?? null,
            ]
        );
        ...
        return {
            ...
            theme: row.theme_primary
                ? { primary: row.theme_primary, secondary: row.theme_secondary, accent: row.theme_accent }
                : null,
...
export async function listShareLinks(projectCode: string): Promise<ShareLink[]> {
    const res = await pool.query(
        `SELECT sl.id, sl.project_id, p.project_code, p.display_name AS project_label, sl.slug, sl.label,
            sl.allowed_pages, sl.theme_primary, sl.theme_secondary, sl.theme_accent, sl.created_at, sl.revoked_at
    ...
    return res.rows.map((r) => ({
        ...
        theme: r.theme_primary
            ? { primary: r.theme_primary, secondary: r.theme_secondary, accent: r.theme_accent }
            : null,
...
export async function verifySharePassword(projectId: string, password: string): Promise<boolean> {
